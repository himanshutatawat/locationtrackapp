...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        editTexttimestamp = findViewById(R.id.editText3);
        ...
        databaseReference.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                try {
                    ...
                    String databasetimestampString = dataSnapshot.child("timestamp").getValue().toString().substring(1, dataSnapshot.child("timestamp").getValue().toString().length()-1);
                    ...
                    String[] stringtimestamp = databasetimestampString.split(", ");
                    Arrays.sort(stringtimestamp);
                    String timestamp = stringtimestamp[stringtimestamp.length-1].split("=")[1];
...
//                    long currentTime = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.getDefault());

                    Date currentTime = new Date(System.currentTimeMillis());
                    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                    String formattedTime = sdf.format(currentTime);
                    editTexttimestamp.setText(formattedTime);


//                    location.setTime(currentTime);
//                    String timestampString = Double.toString(currentTime);
//                    editTexttimestamp.setText(timestampString);
//                    editTexttimestamp.setText(Double.toString(location.setTime(System.currentTimeMillis())));
                } catch (Exception e) {
                    e.printStackTrace();
                }
    ...
    public void updateButtonOnclick(View view){
        ...
        databaseReference.child("timestamp").push().setValue(editTexttimestamp.getText().toString());
...
//                    location.setLatitude(Double.parseDouble(editTextLatitude.getText().toString()));
//                    location.setLongitude(Double.parseDouble(editTextLongitude.getText().toString()));
//                    location.setTime(System.currentTimeMillis()); // Add timestamp to the location object
//
//                    // Save the location object to Firebase Realtime Database
